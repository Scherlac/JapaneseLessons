...
def test_save_creates_file(tmp_path, sample_content):
    path = save_lesson_content(sample_content, _ldir(tmp_path, 1))
    assert path.exists()
...
def test_save_includes_noun_items(tmp_path, sample_content):
    save_lesson_content(sample_content, _ldir(tmp_path, 1))
    loaded = load_lesson_content(1, _ldir(tmp_path, 1))
...
def test_save_creates_parent_directory_if_missing(tmp_path, sample_content):
    nested = tmp_path / "deep" / "nested" / "lesson_001"
    path = save_lesson_content(sample_content, nested)
    assert path.exists()
...
def test_save_lesson_id_zero_padded_three_digits(tmp_path):
    ...
    path = save_lesson_content(content, _ldir(tmp_path, 5))
...
def test_save_lesson_id_large_number(tmp_path):
    ...
    path = save_lesson_content(content, _ldir(tmp_path, 42))
